...
function tinytoml.parse(filename, options)
	...
	while sm.i and sm.i <= sm.input_length do
		sm.byte = sbyte(sm.input, sm.i)

		transition = transitions[sm.mode] and transitions[sm.mode][sm.byte]
		if transition == nil then
			transition = transitions[sm.mode] and transitions[sm.mode][0]
		end

		if not transition then
			sm.mode = dynamic_next_mode or "start_of_line"
			break
		end

		if transition[2] == "?" then
			local candidate = transition[1](sm)

			if type(candidate) == "string" and candidate ~= "" then
				dynamic_next_mode = candidate
			...
			transition[1](sm)
			if type(transition[2]) == "string" and transition[2] ~= "" then
