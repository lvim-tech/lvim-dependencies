...
local POLLING = config.pubspec and config.pubspec.polling or {}
...
function M.set_loading_indicator(bufnr, lnum)
    ...
    local cfg_vt = config.pubspec and config.pubspec.virtual_text
...
--- Clear pubspec caches using the public cache API.
--- Replaces the old direct struct access: cache[TYPE].pubspec.data = {}
function M.clear_global_pubspec_cache()
    cache.clear("pubspec", const.CACHE_TYPES.INSTALLED)
    cache.clear("pubspec", const.CACHE_TYPES.LATEST)
    debug("Cleared global pubspec cache (via cache API)", vim.log.levels.INFO)
end
...
function M.poll_for_outdated(bufnr, name, callback)
    ...
    local function poll()
        ...
        latest.get_package_latest("pubspec", name, function(err, version)
