...
--- Find a package name in a single buffer line (TOML format: "pkg =")
...
function M.format_package_chunks(record)
    ...
    if dep_config and dep_config.format then
        ...
        -- Propagate features/optional/default_features directly from record
        if record.features ~= nil then
            format_data.features = record.features
        end
        if record.optional ~= nil then
            format_data.optional = record.optional
        end
        if record.default_features ~= nil then
            format_data.default_features = record.default_features
        end
