...
local actions = require("lvim-dependencies.managers.go.api")
...
local function handle_install(cmd, callback)
    ...
    if not pkg then
        ui.input("Add Package", "Enter module path", "", function(confirmed, package_name)
...
local function handle_update_direct(cmd, callback)
    ...
    if version then
        actions.update_async(package, { version = version, from_ui = cmd.opts and cmd.opts.from_ui }, callback)
        return
    end
...
local handler = {
    execute = function(cmd, callback)
        ...
        debug(string.format("go handler: %s", cmd.type), vim.log.levels.DEBUG)
        fn(cmd, callback)
    end,
}

operator.register_handler("go", handler)
