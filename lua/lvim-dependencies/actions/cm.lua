...
local ui = require("lvim-dependencies.ui.virtual_text")
...
function M.check_manifest_outdated(bufnr, manifest_key)
	...
	if single_pkg then
		...
		for name in pairs(scalar_deps) do
			state.buffers[bufnr].loading_deps[name] = true
		end
	end
	...
	then
		schedule(function()
			if state.buffers and state.buffers[bufnr] and state.buffers[bufnr].loading_deps then
				for name in pairs(scalar_deps) do
					state.buffers[bufnr].loading_deps[name] = nil
				end
			end
...
function M.invalidate_package_cache(bufnr, manifest_key, package_name)
	...
	local host = registry_hosts[manifest_key]
	if host then
		host_failures[host] = nil
	end

	local deps = state.get_dependencies(manifest_key)
	if deps and deps.outdated and deps.outdated[package_name] then
		deps.outdated[package_name] = nil
		state.set_outdated(manifest_key, deps.outdated)
	end
end
...
function M.clear_buffer_cache(bufnr, manifest_key)
	...
	if cache then
		if cache.watchdog then
			stop_watchdog(cache.watchdog)
			cache.watchdog = nil
		end
		...
		cache.pending = {}
		cache.last_fetched_at = nil
		cache.last_changed = 0
	end

	for key in pairs(negative_cache) do
		if key:match("^" .. manifest_key .. "|") then
			negative_cache[key] = nil
		end
	end
end
