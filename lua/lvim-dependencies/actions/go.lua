...
local function apply_single_line_version_edit(bufnr, lnum1, dep_name, new_version)
    ...
    api.nvim_buf_set_text(bufnr, lnum1 - 1, ver_start - 1, lnum1 - 1, ver_end, { version_to_set })
    vim.bo[bufnr].modified = false
    return true
end
...
function M.fetch_versions(name, _)
    ...
    local current = state.get_installed_version("go", name)
    ...
    vim.system(cmd, { cwd = cwd, text = true }, function(res)
        vim.schedule(function()
            ...
            if not res or res.code ~= 0 then
                ...
                virtual_text.display(bufnr, "go", { force_full = true })
                return
            end
            ...
            vim.system({ "go", "mod", "tidy" }, { cwd = cwd, text = true }, function(res2)
                vim.schedule(function()
                    ...
                    if module_name and version then
                        ...
                        if deps then
                            ...
                            state.set_installed("go", deps.installed)
                        end
                        state.add_installed_dependency("go", module_name, version, "require")
                    end
                    ...
                    vim.defer_fn(function()
                        ...
                        check_manifests.invalidate_package_cache(bufnr, "go", module_name)
                        check_manifests.check_manifest_outdated(bufnr, "go")
                    end, 300)
...
local function run_go_remove(path, name)
    ...
    vim.system({ "go", "mod", "tidy" }, { cwd = cwd, text = true }, function(res)
        vim.schedule(function()
            ...
            vim.defer_fn(function()
                check_manifests.invalidate_package_cache(bufnr, "go", found_name or name)
                check_manifests.check_manifest_outdated(bufnr, "go")
            end, 300)
