...
local function clear_all_caches()
    ...
    composer_parser.clear_lock_cache()
end
...
function M.fetch_versions(name, _)
    ...
    local current = state.get_installed_version("composer", name)
            ...
            if res and res.code == 0 then
                ...
                if name and version then
                    ...
                    if deps then
                        ...
                        state.set_installed("composer", deps.installed)
                    end
                    state.add_installed_dependency("composer", name, version, scope)
                end
                ...
                vim.defer_fn(function()
                    ...
                    check_manifests.invalidate_package_cache(bufnr, "composer", name)
                    check_manifests.check_manifest_outdated(bufnr, "composer")
                end, 300)
                ...
                local function poll_for_outdated()
                    ...
                    if has_fresh_data or poll_count >= max_polls then
                        ...
                        virtual_text.display(bufnr, "composer", { force_full = true })
                    else
    ...
    vim.system(cmd, { cwd = cwd, text = true }, function(res)
        vim.schedule(function()
            ...
            if res and res.code == 0 then
                ...
                state.remove_installed_dependency("composer", name)
                state.set_updating(false)
